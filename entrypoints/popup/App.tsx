...
import { SyncEngine, isLocked, forceReleaseLock, getLockStatus } from '@/lib/sync'
...
function App() {
  ...
  async function loadStatus() {
    ...
    setHasConfig(!!config.githubToken)
    if (config.lastSyncTime) {
      setLastSync(new Date(config.lastSyncTime).toLocaleString())
    }
  }
  ...
  async function handlePush() {
    ...
    try {
      ...
      if (gistId && gistId !== config.gistId) {
        await saveConfig({ gistId })
      }

      setStatus('success')
      setMessage(`已上传 ${result.changes} 个书签`)
      await loadStatus()
    } catch (err) {
      setStatus('error')
  ...
  async function handlePull() {
    ...
    try {
      ...
      setStatus('success')
      setMessage(`已下载 ${result.changes} 处变更`)
      await loadStatus()
  ...
  return (
    <div className="w-[300px] bg-slate-50 flex flex-col font-sans text-gray-900">
      {/* Header */}
      ...
      <div className="p-4 flex flex-col">
        {!hasConfig ? (
          <div className="text-center py-5 bg-white rounded-lg shadow-sm border border-gray-100">
            ...
            <p className="text-xs text-gray-500 mb-3 px-4">请先配置 GitHub Token 以开始同步</p>
            <button
              onClick={openOptions}
              className="px-5 py-1.5 bg-sky-400 hover:bg-sky-500 text-white text-xs font-medium rounded-md transition-all shadow-sm active:scale-95"
            >
              前往设置
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={handlePush}
                disabled={isSyncing}
                className="flex items-center justify-center gap-2 py-3 bg-sky-400 text-white rounded-lg shadow-sm hover:bg-sky-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSyncing && status === 'syncing' && message.includes('上传') ? (
              ...
              <button
                onClick={handlePull}
                disabled={isSyncing}
                className="flex items-center justify-center gap-2 py-3 bg-emerald-400 text-white rounded-lg shadow-sm hover:bg-emerald-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSyncing && status === 'syncing' && message.includes('下载') ? (
                  <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                  </svg>
            ...
            <div className="space-y-2">
              {message && (
                <div className={`flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-medium border ${status === 'error' ? 'bg-[#fef2f2] text-[#991b1b] border-[#fecaca]' :
                  status === 'success' ? 'bg-[#ecfdf5] text-[#065f46] border-[#a7f3d0]' :
              ...
              {lockInfo?.locked && (
                <div className="flex items-center justify-between px-3 py-2 bg-amber-50 text-amber-700 rounded-lg text-xs border border-amber-200">
                  <span>
                    操作锁定中 ({Math.round((lockInfo.elapsed || 0) / 1000)}秒)
                  </span>
                  <button
